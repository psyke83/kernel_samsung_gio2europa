...
#ifdef CONFIG_RPCSERVER_HANDSET
...
#define MSM_PMEM_ADSP_SIZE 	0x86E000	// 3M :0x86E000, 2M : 0x77F000 
#define MSM_PMEM_AUDIO_SIZE 	0x5B000 
...
/******BT & WIFI GPIO definition **********/
#if defined(CONFIG_MACH_LUCAS)
#define	GPIO_WLAN_HOST_WAKE	17
#define	VWLAN_EN				77
...
static int bluetooth_power(int on)
{
	...
	if (on) {
		...
		if (gpio_get_value(WLAN_RESET_N) == 0)
			wlan_setup_clock(1);
...
#endif
	GPIO_CFG(1,  0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA), /* SEPCTRUM IC FOR BENNETT*/
...
#endif
	GPIO_CFG(1,  0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), /* SPECTRUM IC FOR BENNETT */
...
static void sdcc_gpio_init(void)
{
	...
	if (gpio_request(VWLAN_EN, "wlan_en"))
		pr_err("failed to request gpio wlan_en\n");
	if (gpio_request(WLAN_RESET_N, "WLAN_RESET_N"))
		pr_err("failed to request WLAN_RESET_N\n");
...
{
	int rc;
	rc = gpio_get_value(WLAN_RESET_N);
...
void wlan_setup_power(int on, int detect)
{
	...
	if (on) {
		...
		gpio_set_value (WLAN_RESET_N, 1);	/* WLAN_RESET */
#ifdef WLAN_HOST_WAKE
		wlan_host_wake_init();
#endif /* WLAN_HOST_WAKE */
	}
		...
		wlan_host_wake_exit();
#endif /* WLAN_HOST_WAKE */
		gpio_set_value (WLAN_RESET_N, 0);	/* WLAN_RESET */
...
static void __init msm7x2x_init_mmc(void)
{
	...
	sdcc_gpio_init();
	if (gpio_tlmm_config(GPIO_CFG(VWLAN_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE))
		printk (KERN_ERR "Error configuraing VWALN_EN\n");
	if (gpio_tlmm_config(GPIO_CFG(WLAN_RESET_N, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE))
